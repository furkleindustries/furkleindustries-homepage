window.storyFormat({
    description: "its the furkle format baby",
    author: 'furkle',
    image: 'icon.svg',
    name: 'Gately',
    url: 'http://FURKLEINDUSTRIES.com',
    version: '69.42.0',
    proofing: false,
    source:
        '<!DOCTYPE html>\n' +
        '<html lang="en" class="unloaded">\n' +
            ...
            '<body>\n' +
                ...
                '<script id="gatelyInit" src="/Applications/Twine 2.app/Contents/Resources/app.nw/story-formats/gately/gatelyBuilt.js"></script>\n' +
            '</body>\n' +
        '</html>',
    setup: function() {
        'use strict';
        ...
        const Matches = {
            ...
            currentlyParsingString() {
                return Matches.currentMatch &&
                    (Matches.currentMatch.dataType === 'singleQuote' ||
                        Matches.currentMatch.dataType === 'doubleQuote');
            },
        ...
        const Parser = {
            ...
            getMacroType(text, id) {
                let fragment = text.slice(
                    text.indexOf('<tw-match data-gately-id="' + id + '"'));
                fragment = fragment.slice(
                    fragment.indexOf('data-macro-type="') + 17);
                let macroType = fragment
                    .slice(0, fragment.indexOf('"'))
                    .trim()
                    .toLowerCase();
                return macroType;
            },
            ...
            parseNode(node) {
                ...
                if (!node || typeof(node.innerHTML) === 'undefined') {
                    wideAlert('The node argument to be parsed is not a valid HTML ' +
                        'element.',
                        node);
                    return;
                }

                const savedSSText = $(node).find('script, style')
                    .toArray()
                    .map(function(elem) {
                ...
                while (match = regex.exec(text)) {
                    text = text.replace(match[0], 
                        match[0].slice(0, match[0].length - 1) + '&gt;');
                }
                ...
                for (let ii = 0; ii < childs.length; ii++) {
                    const child = childs[ii];
                    // handle exact duplicates
                    index += text.slice(index).indexOf(child.outerHTML);
                    
                    const start = index;

                    if (child.tagName === 'TW-ARGUMENT') {
                        nodes.push(text.slice(start, start + child.outerHTML.length));
                    } else {
                        let newText =
                            Parser.createOpenTag(Matches.getNewMatchId(), 'html');
                        // add new text
                        text = Parser.addStringAtIndex(text, newText, index);
                        // add the length of the new text
                        index += newText.length;

                        index += child.outerHTML.length;
                ...
                while (ii < text.length) {
                    ...
                    if (text[ii] === '"' || text[ii] === "'") {
                        ...
                        // backslashes
                        let current = Matches.currentMatch;
                        if (!current ||
                            Matches.getDataType(current) === 'html' ||
                            Matches.getDataType(current) === 'hook' ||
                            Matches.getDataType(current) === otherType ||
                            text[ii - 1] === '\\' ||
                        ...
                        while (ii < text.length) {
                                ...
                                backslashes % 2 === 0)
                            {
                                ...
                                let after = text.slice(ii);
                                let nodeMatches = nodes.map(function(aa) {
                                    return after.slice(0, aa.length) === aa;
                                });

                                for (let jj = 0; jj < nodeMatches.length; jj++) {
                                    if (nodeMatches[jj]) {
                                        ii += nodes[jj].length - 1;
                                        origIndex += nodes[jj].length - 1;
                                        break;
                                    }
                                }
                            } else {
                        ...
                        if (ii !== text.length) {
                            newText = '</tw-match>';
                            text = Parser.addStringAtIndex(text, newText, ii + 1);
                            ii += newText.length;
                            matchOrigIndexes.shift();
                        }
                    } else if (Matches.currentlyParsingString()) {
                        ii++;
                        origIndex++;
                        continue;
                    } else if (text[ii] === '<') {
                    ...
                    } else if (text.slice(ii, ii + 4) === '&lt;' &&
                        text.slice(ii + 4, ii + 8) === '&lt;')
                    {
                        // macros names are terminated by a space or &gt;&gt;
                        let macroType = text.slice(ii + 8).split(/ |&gt;&gt;/)[0];
                        if (!macroType) {
                            ii++;
                            origIndex++;
                            continue;
                        ...
                        origIndex += macroType.length;

                        // don't start a new macro if it's an ending tag
                        if (Matches.currentMatch) {
                            let currentMatchType = 
                                Matches.getMacroType(Matches.currentMatch);
                            ...
                            if (Parser.isEndingTag(macroType)) {
                                const amt = text.indexOf('&gt;&gt;', ii);
                                if (amt === -1) {
                                    ii++;
                                    origIndex++;
                                    continue;
                                } else {
                                    const close = Parser.createOpenTag(
                                        Matches.getNewMatchId(),
                                        'closeTagTerminator',
                                        'sugarcube');
                                    text = Parser.addStringAtIndex(text, close, ii);
                                    ii += close.length;
                                }
                            ...
                            const newMatch = Matches.createMatchObject(
                                'macro',
                                Matches.currentMatch,
                                'sugarcube',
                                macroType);
                            ...
                            Matches.currentMatch = Matches.createMatchObject(
                                'macro',
                                null,
                                'sugarcube',
                                macroType);
                        ...
                        {
                            if (Matches.currentMatch &&
                                Matches.currentMatch.dialect === 'harlowe')
                            {
                        ...
                        let macroType = text.slice(ii + 1).split(':')[0];
                        if (!macroType) {
                            ii++;
                            origIndex++;
                            continue;
                        ...
                        if (Matches.currentMatch) {
                            let newMatch = Matches.createMatchObject(
                                'macro',
                                Matches.currentMatch,
                                'harlowe',
                                macroType);
                        ...
                        text.slice(ii + 4, ii + 8) === '&gt;')
                    {
                        ...
                        const currentMatch = Matches.currentMatch;
                        const closing = /&lt;&lt;\/\S+&gt;&gt;|&lt;&lt;end\S+&gt;&gt;/;
                        if (closing.test(text.slice(ii))) {
                        ...
                        if (!Matches.currentMatch.finishedStartTag) {
                            ...
                            const parent = 
                                Matches.getCurrentParent(Matches.currentMatch);
                            Matches.currentMatch = parent;
                            matchOrigIndexes.shift();
                        }
                    } else if (text[ii] === ')') {
                        if (Matches.harloweParentheses > 0) {
                            ...
                            Matches.currentMatch.dialect === 'sugarcube')
                        {
                            ii++;
                            origIndex++;
                            continue;
                        ...
                        const currentMatch = Matches.currentMatch;
                        let type = Matches.getMacroType(currentMatch) || '';
                        let macroObj = Matches.getMacroObject(type);
                        ...
                        let newText;
                        let nextHook = text.slice(ii).match(/\s*\[/);
                        if (nextHook &&
                            text.slice(ii).indexOf(nextHook[0]) === 0 &&
                        ...
                        let newText = Parser.createOpenTag(
                            Matches.getCurrentMatchId(), 'hook', 'harlowe');
                        // add new text
                        text = Parser.addStringAtIndex(text, newText, ii);
                        // add the length of the new text
                        ii += newText.length;
                        ...
                        const hookID = Matches.getCurrentMatchId();
                        ii++;
                        ...
                        let type = Matches.getMacroType(Matches.currentMatch);
                        if (type &&
                            Matches.isFinishedStartTag(Matches.currentMatch))
                        {
                            newText = '</tw-match>';
                            text = Parser.addStringAtIndex(text, newText, ii + 1);
                            ii += newText.length;
                        ...
                        let newText = Parser.createOpenTag(
                            Matches.getNewMatchId(), 'variable');
                        text = Parser.addStringAtIndex(text, newText, ii);
                        ii += newText.length + 1;
                        ...
                        ii += newText.length - 1;
                    } else if (text[ii].match(/\d/)) {
                        // skip anything outside of matches
                        if (!Matches.currentMatch ||
                            Matches.isFinishedStartTag(Matches.currentMatch) ||
                            Matches.getDataType(Matches.currentMatch) === 'hook')
                        {
                            ii++;
                            origIndex++;
                            continue;
                        ...
                        let temp = text.slice(ii).search(/\D/);
                        const firstPeriod = text.slice(ii).search(/\./);

                        if (temp === -1) {
                            temp = text.slice(ii).length;
                        } else if (firstPeriod !== -1 && firstPeriod <= temp) {
                            temp += text.slice(ii).match(/\.\d*/)[0].length;
                        }

                        /* skip anything outside of matches, or invalid matches,
                         * or that's inside valid quotes */
                        if (!text[ii + temp] ||
                            /[^\d.,]/.test(text.slice(ii, ii + temp)) ||
                            !/^([\s+\-*\/%(),>]|&gt;)/.test(text.slice(ii + temp)))
                        {
                            ii++;
                            origIndex++;
                            continue;
                        ...
                        let newText = Parser.createOpenTag(
                            ...
                            'number');
                        text = Parser.addStringAtIndex(text, newText, ii);
                        ii += temp + newText.length;
                        origIndex += temp;

                        newText = '</tw-match>';
                        text = Parser.addStringAtIndex(text, newText, ii);
                        ii += newText.length - 1;
                    } else if (text[ii].match(/[^\s<>]/)) {
                        // skip anything outside of matches
                        if (!Matches.currentMatch ||
                            Matches.isFinishedStartTag(Matches.currentMatch) ||
                            Matches.getDataType(Matches.currentMatch) === 'hook')
                        ...
                        if (text[ii] === ',' &&
                            Matches.currentMatch.dataType === 'macro')
                        {
                            let newText = Parser.createOpenTag(
                                Matches.getNewMatchId(),
                                'comma');
                        ...
                        let word = text.slice(ii).split(/[\s<>)]|&lt;|&gt;/)[0];
                        if (text[ii - 1].match(/\s/) && 
                ...
                const selector =
                    'tw-match[data-type="macro"][data-dialect="sugarcube"] >' +
                    'tw-match[data-type="openTagTerminator"]';
                const temp = node.querySelectorAll(selector);
                const full = [];
                // reverse the array – no Array.from in nwjs?
                for (let ii = 0; ii < temp.length; ii++) {
                    full.unshift(temp[ii]);
                }

                for (let ii = 0; ii < full.length; ii++) {
                    const terminator = full[ii];
                    let next = terminator.nextSibling;
                    if (next) {
                        const foo = document.createElement('tw-match');
                        foo.setAttribute('data-type', 'hook');
                        next.parentElement.insertBefore(foo, next);
                        
                        const _name = 'closeTagTerminator';
                        while (next &&
                            (next.dataType !== 1 ||
                                next.getAttribute('data-type') !== _name))
                        {
        ...
        CodeMirror.defineMode('gately', function() {
            let cm;
            let tree;
            ...
            let init = function() {
                const doc = cm.doc;
                /*
                    Use the Gately parser to compute a full parse tree.
                */

                const selector = 'tw-gately-cm-container';

                tree = document.createElement(selector);
                tree.textContent = doc.getValue();
                tree.dataFixed = false;
                ...
                */
                ...
                let change;
                doc.on('beforeChange', function(_, _change) {
                    change = _change;
                });

                const noBounce = debounce(function() {
            ...
            return {
                startState() {
                    if (!cm) {
                        /*
                            TwineJS's PassageEditor stores a reference to the CodeMirror instance in
                            the Gately modes object.
                        */
                        cm = CodeMirror.modes.gately.cm;
                    }
                ...
                token: function token(stream, state) {
                            ...
                            if (txt.length > 1 && /\n/.test(txt)) {
                                ...
                                for (let ii = 0; ii < txt.length; ii++) {
                                        ...
                                        !full.length)
                                    {
                                        ...
                                        full[full.length - 1] += txt[ii];
                                    }
                                }

                                full = full.map(function(aa) {
                                    const span = document.createElement('span');
                                    span.textContent = aa;
                                    return span;
                                }).forEach(function(aa) {
                                    current.parentElement.insertBefore(aa, current);
                                });
                    ...
                    while (nodeList[index]) {
                        count += nodeList[index].textContent.length;
                    ...
                    const stripped = currentNode.textContent;
                    ...
                    while (node &&
                        node.tagName !== 'TW-GATELY-CM-CONTAINER')
                    {
                        if (!node) {
                            break;
                        }
                        ...
                        let name = 'gately-' + type;
                        counts[name] = (counts[name] || 0) + 1;

                        if (counts[name] > 1 && type !== 'html') {
                            name += '-' + (counts[name]);
                        }
        ...
        let gatelyStyles = document.querySelector('style#cm-gately');
        if (!gatelyStyles) {
            gatelyStyles = document.createElement('style');
            gatelyStyles.setAttribute('id', 'cm-gately');
            gatelyStyles.innerHTML =
                '.cm-gately-macro {' +
                    'color: rgb(218, 119, 51);' +
                '}' +
                '.cm-gately-variable {' +
                    'color: rgb(31, 198, 99);' +
                '}' +
                '.cm-gately-singleQuote, .cm-gately-doubleQuote {' +
                    'color: rgb(218, 51, 150);' +
                '}' +
                '.cm-gately-comma {' +
                    'color: rgb(0, 0, 0);' +
                '}' +
                '.cm-gately-hook {' +
                    'background: rgba(51, 150, 218, 0.05);' +
                '}' +
                '.cm-gately-hook-2 {' +
                    'background: rgba(51, 150, 218, 0.10);' +
                '}' +
                '.cm-gately-hook-3 {' +
                    'background: rgba(51, 150, 218, 0.20);' +
                '}' +
                '.cm-gately-hook-4 {' +
                    'background: rgba(51, 150, 218, 0.25);' +
                '}' +
                /*'.cm-gately-hook-5 {' +
                    'background: rgba(51, 150, 218, 0.375);' +
                '}' +*/
                '.cm-gately-reservedWord {' +
                    'color: red;' +
                '}' +
                '.cm-gately-number {' +
                    'color: blue;' +
                ...
                '.cm-gately-html {' +
                    'color: rgb(218, 51, 66);' +
                '}';
            document.head.appendChild(gatelyStyles);
        }
    }
});